...
class VtkConan(ConanFile):
    ...
    url = "https://github.com/paulharris/conan-vtk"
    ...
    # NO SUBSOURCE # # exports = ["LICENSE.md", "CMakeLists.txt", "FindVTK.cmake"]
    ...
    default_options = {
            ...
            , "download_vtk_git": False # use the tarball
    ...
    def source(self):
        if self.options.download_vtk_git:
            self.run("git clone -b release --single-branch " + self.git_url + " " + self.vtk_source_folder)
            # note: we give the branch a name so we don't have detached heads
            # TODO change to standard git and python chdir
            self.run("cd " + self.vtk_source_folder + " && git checkout -b branch-" + self.git_hash + " " + self.git_hash)
        else:
            get(**self.conan_data["sources"][self.version],
                    strip_root=True,
                    destination=self.vtk_source_folder,
                    )
        ...
        remove_files_by_mask(os.path.join(self.vtk_source_folder,"CMake"), "Find*.cmake")
        ...
        rmdir(self, os.path.join(self.vtk_source_folder,"CMake","patches"))
    ...
    def build_requirements(self):
        if self.options.rendering:
            Apt(self).install([
                # TODO check these are the packages we still need
                "freeglut3-dev",
                "mesa-common-dev",
                "mesa-utils-extra",
                "libgl1-mesa-dev",
    ...
    def generate(self):
        ...
        tc.variables["VTK_ENABLE_KITS"] = "ON"
